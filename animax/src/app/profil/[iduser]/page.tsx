...
export default async function ProfilPublicPage({ params }: { params: { iduser: string } }) {
  ...
  if (isNaN(iduser)) {
    return <div className="p-8 text-center">Profil invalide.</div>;
  }
          ...
          {user.note.length === 0 ? (
            ...
            <ul className="space-y-4">
              {user.note.map((n: any, idx: number) => (
                <li key={`${n.idnote}-${n.anime?.contenu ?? idx}`} className="flex items-center gap-4 bg-purple-950 p-4 rounded-md">
                  {n.anime?.file?.url && (
                    <Image src={n.anime.file.url.startsWith("/") ? n.anime.file.url : `/${n.anime.file.url}`} alt={n.anime.contenu} width={48} height={64} className="rounded" />
                  )}
                  <div>
